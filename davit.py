...
class DaViT(nn.Module):
    ...
    def __init__(self, embed_dims=256, num_heads=8, window_size=8
                 ):
        super().__init__()
        ...
        self.da_norm = nn.LayerNorm(self.embed_dims)
    def forward(self, x):
        B, C, H, W = x.shape
        ...
        out = self.da_norm(x)
        print(out.shape)
        out = out.transpose(-1, -2).reshape(B, self.embed_dims, H, W)
        print(out.shape)
        return out

if __name__ == '__main__':
    x = torch.randn((32,256,16,16))
    model = DaViT()
    out = model(x)
    # print(model)
    model_dict = model.state_dict()
    print(model_dict.keys())
    checkpoint = torch.load('iter_160000.pth')  # 加载模型
    ch = checkpoint['state_dict']
    # for k in ch.keys():
    #     if 'backbone.patch_embeds.3' in k or 'backbone.main_blocks.3' in k or 'backbone.norm3' in k:
    #         print(k)
    # for k, v in ch.items():
    # print(k)
    pretrained_dict = {k[9:]: v for k, v in ch.items() if 'backbone.patch_embeds.1' in k\
            or 'backbone.main_blocks.1' in k or 'backbone.norm1' in k}
    print(pretrained_dict.keys())
    print(len(model_dict), len(pretrained_dict))
    index = 0
    for m_key,p_key in zip(model_dict.keys(),pretrained_dict.keys()):
        if 'Block' in m_key or 'da_norm' in m_key:
            model_dict[m_key] = pretrained_dict[p_key]
        index =index + 1
    print(index)
    # print(model_dict)
