...
from models.model_cgb_hor import MCM_Hit_mor
import os
import time
...
'''
python test_single_each_epoch_sy_no_hd95.py  --loss_type CE+Dice_config_cgb_davit --window_size 8 --patch_size 1 
'''
...
max_dice = 0
for weight_epoch in range(1,len(os.listdir(weigth_input_path))+1):
    print('epoch',weight_epoch)
    #======================================load model==========================================
    weigth_path = weigth_input_path + str(weight_epoch)+'.pth'
    # print(weigth_path)
    ...
    for i in range(test_len):
        ...
        with torch.no_grad():
            for image_input, label_input in tqdm(test_loader):      #single case slice
                ...
                output = sliding_window_inference(image_input,(image_size,image_size),8,model,overlap=0.25)
    ...
    logger.info('dice {}'.format(Index[0]))

    # print(best_metric_list)
    if max_dice < Index[0]:
        max_dice = Index[0]
        logger.info('best epoch: {}, max dice: {} '.format(weight_epoch,max_dice))
        save_csv = pd.DataFrame(best_metric_list, columns=indicator_list)
        save_csv.insert(0, 'Class', label_list)
        save_csv.insert(len(indicator_list)+1, 'Best_eopch', weight_epoch)
        save_csv.to_csv(save_path+dataset_type+'.csv',index=False,sep=',')
        logger.info('save'+save_path+dataset_type+'.csv')
    print('\n')
