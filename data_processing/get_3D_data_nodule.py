...
def get_3D_patch_slice(data_image, data_masks, patch_size, crop_size,
    ...
    for i in range(z):
        if label[:, :, i].max() == 1:
            ...
            mask = pad_label[:, :, index].astype(np.int64)
    ...
    if len(no_tumor) <= 0:
        ...
        no_tumor_number = int(len(no_tumor) * (1/3) )  ##取不包含tumor数量（4倍）
...
if __name__ == "__main__":
    '''
    python get_3D_data_nodule.py  --patch_size 3
    '''
    import argparse
    ...
    train_list = load_file_name_list(os.path.join(main_path, 'train_88.txt'))
    val6_list = load_file_name_list(os.path.join(main_path, 'val_6.txt'))
    val12_list = load_file_name_list(os.path.join(main_path, 'val_12.txt'))
    
    
    input_path_88 = main_path + 'LungNoduleLIDCTop100/02Normalized_subsetUR/'
    ...
    for Num in range(3):
        if Num == 0:
            ...
            File = 'train'
            input_path = input_path_88
            output_path = main_path + 'train_data/' + patch_type + '/images/'
        elif Num == 1:
            select_name = 'test'
            File_list = val12_list
            File_leng = len(val12_list)
            File = 'val'
            input_path = input_path_88
            output_path = main_path + 'test_data/' + patch_type + '/'
        elif Num == 2:
            select_name = 'test'
            File_list = val6_list
