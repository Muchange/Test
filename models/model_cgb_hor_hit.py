...
from .model_parts_cgb_hor_hit import *
...
class MCM_Hit_mor(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, window_size=(8,8)):
        ...
        drop_path_rate_cgb = 0.3
        drop_path_rate_gnconv = 0.1
        drop_cgb = [x.item() for x in torch.linspace(0, drop_path_rate_cgb, 4)]
        drop_gnconv = [x.item() for x in torch.linspace(0, drop_path_rate_gnconv, 4)]
        hornet_depths = [1,1,1,1]
        gnconv_hw = [16,8,8,4]
        order = [2,3,4,5]
        gflayer = True
        ...
        self.down1 = Hornet_down(in_channels=num_channels[0], out_channels=num_channels[1],
                                 ...
                                 order=order[0], grid_size = grid_size,drop = drop_gnconv[0]
                                 )
        self.down2 = Hornet_down(in_channels=num_channels[1], out_channels=num_channels[2],
                                 Hornet_depths=hornet_depths[1],
                                 order=order[1], grid_size = grid_size,drop = drop_gnconv[1]
                                 )
        self.down3 = Hornet_down(in_channels = num_channels[2], out_channels = num_channels[3],
                                 Hornet_depths=hornet_depths[2],
                                 order=order[2], grid_size = grid_size,drop = drop_gnconv[2]
                                 )    #B,192,32,32

        self.down4 = Hornet_down(in_channels = num_channels[3], out_channels = num_channels[4],
                                 Hornet_depths=hornet_depths[3],
                                 order=order[3], grid_size = grid_size,drop = drop_gnconv[3]
                                 )    #B,256,16,16

        self.CGB1 = CGB(num_channels[0],num_channels[1], drop_cgb[0], grid_size) #256 + 128，256：down
        self.CGB2 = CGB(num_channels[1],num_channels[2], drop_cgb[1], grid_size)  #128 + 64，64：upsample
        self.CGB3 = CGB(num_channels[2],num_channels[3], drop_cgb[2], grid_size)  #64 + 32，32：upsample
        self.CGB4 = CGB(num_channels[3],num_channels[4], drop_cgb[3], grid_size)  #64 + 32，32：upsample
        ...
        # self.up5 = de_conv(in_channels=num_channels[1], out_channels=num_channels[0],
        #                     up_in_channels=up_num_channels[0])  # B,192,32,32
