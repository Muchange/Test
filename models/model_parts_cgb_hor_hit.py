...
# from hornet import *
from .hornet_hit import Block, gnconv, HorNet, GlobalLocalFilter, LayerNorm
...
class Hornet_Block(nn.Module):
    def __init__(self, in_channels, Hornet_depths, order, grid_size ,
        ...
        self.stage = nn.Sequential(
            *[Block(dim=in_channels, drop_path=drop_path_rate, order=order, s=s, grid_size = grid_size,
                    layer_scale_init_value=layer_scale_init_value) for j in range(Hornet_depths)]
        )
        cur += Hornet_depths
...
class Hornet_down(nn.Module):
    ...
    def __init__(self, in_channels=128, out_channels=256, Hornet_depths=[18],
        ...
        self.down_hornet = Hornet_Block(in_channels=out_channels, Hornet_depths=Hornet_depths,
                                        order=order, grid_size = grid_size,
                                        drop_path_rate=drop, layer_scale_init_value=1e-6)
    ...
    def forward(self, x):
        ...
        out = self.downsample_layers(out)
        # print(out.shape)
...
class Hornet_up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_in_channels, Hornet_depths,
                 order, grid_size,drop = 0.1):
        super().__init__()
        self.up = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2)

        self.up_conv = nn.Sequential(
            nn.BatchNorm2d(up_in_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(up_in_channels, out_channels, kernel_size=3, stride=1, padding=1),
