...
class Inconv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.inconv =  Conv_residual(in_channels, out_channels)
    def forward(self, x):
        # print(x.shape)
        out = self.inconv(x)
        # print(out.shape)
...
class merge_c2f_d(nn.Module):
    def __init__(self, in_channels,embed_dims,out_channels,drop_path,type):
        super().__init__()
        self.type = type
        # self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0)
        self.van = van_Block(in_chans = in_channels, embed_dims = embed_dims,out_chans = out_channels, drop_path=drop_path)
...
class merge_c2f_t(nn.Module):
    def __init__(self, in_channels,embed_dims,out_channels,drop_path):
        super().__init__()
