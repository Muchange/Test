...
class merge_c2f_d(nn.Module):
    def __init__(self, in_channels,out_channels,type,Conv):
        ...
        if Conv == 'dwse':
            ...
            self.dwconv_se = nn.Conv2d(out_channels * 2,out_channels,kernel_size=3,padding=1)
        elif Conv == 'lsc1x1':
...
class merge_c2f_t(nn.Module):
    def __init__(self, down_channels,up_channels,out_channels,Conv):
        ...
        if Conv == 'dwse':
            ...
            self.dwconv_se = nn.Conv2d(out_channels * 3,out_channels,kernel_size=3,padding=1)
        elif Conv == 'lsc1x1':
            self.dwconv_se = nn.Sequential(
