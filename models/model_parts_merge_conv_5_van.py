...
class van(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(in_channels, in_channels, 7, stride=1, padding=9, groups=in_channels, dilation=3),
...
class merge_c2f_d(nn.Module):
    def __init__(self, in_channels,out_channels,type):
        ...
        self.dwconv_van = van(out_channels * 2,out_channels)
    def forward(self, x1, x2):
        x2 = self.conv_change(x2)
        merge = torch.cat([x1,x2], dim=1)
        # print(merge.shape)
        # merge = self.conv(merge)
        out = self.dwconv_van(merge)
        return out
...
class merge_c2f_t(nn.Module):
    def __init__(self, down_channels,up_channels,out_channels):
        ...
        self.dwconv_van = van(out_channels * 3,out_channels)
