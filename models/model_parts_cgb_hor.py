...
# from hornet import *
from .hornet import Block, gnconv,  GlobalLocalFilter, LayerNorm
...
class Hornet_Block(nn.Module):
    def __init__(self, in_channels, Hornet_depths, order, gnconv_h, gnconv_w,
                    grid_size,
                 drop_path_rate=0.1, layer_scale_init_value=1e-6, gflayer=False):
        super().__init__()
        s = 1.0 / 3.0
        ...
        self.stage = nn.Sequential(
            *[Block(dim=in_channels, drop_path=dp_rates[cur + j], order=order, s=s, 
                    h=gnconv_h, w=gnconv_w,grid_size =grid_size,
                    layer_scale_init_value=layer_scale_init_value, gflayer=gflayer) for j in range(Hornet_depths)]
        )
        cur += Hornet_depths
...
class inConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv_res = Conv_residual(in_channels, out_channels)
...
class de_conv(nn.Module):
    ...
    def __init__(self,in_channels, out_channels, up_in_channels ):
        ...
        self.conv_ch = nn.Conv2d(up_in_channels, out_channels, kernel_size=1)
        self.upconv = Conv_residual(out_channels, out_channels)
...
class Hornet_down(nn.Module):
    ...
    def __init__(self, in_channels=128, out_channels=256, Hornet_depths=[18],
                 order=4, gnconv_h=14, gnconv_w=8, grid_size =8,gflayer=False
                 ):
        super().__init__()
        # print('--------------')
        ...
        self.down_hornet = Hornet_Block(in_channels=out_channels, Hornet_depths=Hornet_depths,
                                        order=order, gnconv_h=gnconv_h, gnconv_w=gnconv_w,grid_size =grid_size,
                                        drop_path_rate=0.1, layer_scale_init_value=1e-6, gflayer=gflayer)

        # self.downsample_conv = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0)

    def forward(self, x):
        # out = self.downsample(x)
        out = self.downsample_layers(x)
        # print(out.shape)
        out = self.down_hornet(out)
        return out
...
class Hornet_up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_in_channels, Hornet_depths,
        ...
        self.up_conv = nn.Conv2d(up_in_channels, out_channels, kernel_size=1, padding=0)
        ...
        self.upsample_layers = nn.Conv2d(out_channels, out_channels, kernel_size=1, padding=0)
