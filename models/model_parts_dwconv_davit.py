...
class low_davit(nn.Module):
    def __init__(self, dim,embed_dim, num_heads,window_size,depth,drop_path):
        super().__init__()
        window_size = window_size[0]
        self.embed_dim = embed_dim
        self.patch = PatchEmbed(dim * 2, self.embed_dim)
...
class merge_dwconv(nn.Module):
    def __init__(self, first_num_channels,num_channels):
        super().__init__()
        self.down = Down_sample_conv(first_num_channels, num_channels)
        self.conv2former = conv2former(num_channels * 2)
        self.conv_out = nn.Conv2d(num_channels * 2, num_channels, kernel_size=3,padding=1,bias=True)
    def forward(self, x1, x2):
        ...
        # print(merge.shape)
        merge = self.conv2former(merge)
        out = self.conv_out(merge)
        return out


class merge_davit(nn.Module):
    def __init__(self,first_num_channels,num_channels,embed_dim, num_heads,window_size,depth,drop):
        super().__init__()
        self.down = Down_sample_conv(first_num_channels,num_channels)
        self.davit = low_davit(dim=num_channels, embed_dim = embed_dim,num_heads = num_heads,
                                window_size= window_size,depth = depth,drop_path = drop)
...
class cat_skip(nn.Module):
    def __init__(self,first_num_channels,num_channels):
        ...
        # print(num_heads)
        self.down = Down_sample_conv(first_num_channels,num_channels)
        self.conv = nn.Conv2d(num_channels*2,num_channels, kernel_size=3,padding=1,bias = True)
    def forward(self, x1, x2):
        # print(x1.shape,x2.shape)
        x2 = self.down(x2)
        assert x1.shape[1] == x2.shape[1]
