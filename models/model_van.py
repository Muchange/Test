...
class MCM_Hit_mor(nn.Module):
    def __init__(self, n_channels=1, n_classes=9, window_size=(8,8)):
        ...
        num_channels = [32,64,128,320,512]
        ...
        self.merge1 = merge_c2f_d(in_channels = merge_channels[0], embed_dims = embed_dim[1],out_channels = num_channels[1],
                                  mlp_ratios =mlp_ratios[0],drop_path = drop_rate[0],type= 'up') #256 + 128，256：down
        self.merge2 = merge_c2f_t(in_channels = merge_channels[1], embed_dims = embed_dim[2],out_channels = num_channels[2],
                                  mlp_ratios =mlp_ratios[1],drop_path = drop_rate[1])  #128 + 64，64：upsample
        self.merge3 = merge_c2f_t(in_channels = merge_channels[2], embed_dims = embed_dim[3],out_channels = num_channels[3],
                                  mlp_ratios =mlp_ratios[2],drop_path = drop_rate[2])
        self.merge4 = merge_c2f_d(in_channels = merge_channels[3], embed_dims = embed_dim[4],out_channels = num_channels[4],
                                  mlp_ratios =mlp_ratios[3],drop_path = drop_rate[3],type= 'down')
        #
    ...
    def forward(self, x):
        ...
        up4 = self.up4(merge4, merge3)
        up3 = self.up3(up4, merge2)
        up2 = self.up2(up3, merge1)
        up1 = self.up1(up2, x1)
        out = self.outc(up1)
        return out
