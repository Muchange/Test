...
class gnconv(nn.Module):
    def __init__(self, dim, order=5,drop_path = 0.1, grid_size = 8, s=1.0):
        super().__init__()
        self.order = order
        ...
        # print(drop_path)
        self.hit = HiT(sum(self.dims), drop_path, grid_size)
    ...
    def forward(self, x, mask=None, dummy=False):
        ...
        dw_abc = self.hit(abc) * self.scale
...
class Block(nn.Module):
    r""" HorNet block"""
    def __init__(self, dim,order, s, grid_size , drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        # print(type(dim))
        self.norm1 = LayerNorm(dim, eps=1e-6, data_format='channels_first')
        
        self.gnconv_hit = gnconv(dim = dim,order = order, s=s, grid_size=grid_size ,
                                 drop_path=drop_path)
    ...
    def forward(self, x):
        ...
        if self.gamma1 is not None:
            ...
            gamma1 = 1
        x = x + self.drop_path(gamma1 * self.gnconv_hit(self.norm1(x)))
...
class HorNet(nn.Module):
    def __init__(self, in_channels=128, out_channels=1000,
        ...
        self.norm = nn.LayerNorm(dims[self.Len-1], eps=1e-6) # final norm layer
        self.head = nn.Linear(dims[self.Len-1],out_channels)
