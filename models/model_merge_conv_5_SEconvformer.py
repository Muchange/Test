...
from .model_parts_merge_conv_5_SEconvformer import *
...
class MCM_Hit_mor(nn.Module):
    def __init__(self, n_channels=1, n_classes=9, window_size=(8,8)):
        ...
        up_num_channels = [num_channels[0] * 2,
                           ...
                           num_channels[2] * 2,
                           num_channels[3] * 2]
        cat_channels = [num_channels[0] + num_channels[1],
        ...
        self.merge1 = merge_c2f_d(in_channels = num_channels[1], out_channels = num_channels[0],
                                cat_channels = cat_channels[0],type= 'up') #256 + 128，256：down
        self.merge2 = merge_c2f_t(down_channels = num_channels[0], up_channels = num_channels[2],
                                cat_channels = cat_channels[1],out_channels = num_channels[1])                                             #128 + 64，64：upsample
        self.merge3 = merge_c2f_t(down_channels = num_channels[1], up_channels = num_channels[3],
                                cat_channels = cat_channels[2],out_channels = num_channels[2])
        self.merge4 = merge_c2f_t(down_channels = num_channels[2], up_channels = num_channels[4],
                                cat_channels = cat_channels[3],out_channels = num_channels[3])
        self.merge5 = merge_c2f_d(in_channels = num_channels[3], out_channels = num_channels[4],
                                cat_channels = cat_channels[4],type= 'down')
