...
class MCM_Hit_mor(nn.Module):
    def __init__(self, n_channels=1, n_classes=9, window_size=(8,8)):
        ...
        self.CGB1 = cross_conv(first_num_channels = num_channels[0],num_channels = num_channels[1]) #256 + 128，256：down
        self.CGB2 = cross_conv(first_num_channels = num_channels[1],num_channels = num_channels[2])  #128 + 64，64：upsample
        self.CGB3 = cross_conv(first_num_channels = num_channels[2],num_channels = num_channels[3])  #128 + 64，64：upsample
        # self.CGB3 = cross_davit(first_num_channels = num_channels[2],num_channels = num_channels[3], 
        #                         num_heads = num_heads[0],embed_dim = embed_dim[0],
        #                         window_size = window_size,depth = depth[0],drop_path = drop[0])
        self.CGB4 = cross_davit(first_num_channels = num_channels[3],num_channels = num_channels[4], 
                                num_heads = num_heads[1],embed_dim = embed_dim[1],
                                window_size = window_size,depth = depth[1],drop_path = drop[1])
        ...
        self.out_dwconv = dw_conv(num_channels[0])
