...
class merge_c2f_d(nn.Module):
    def __init__(self, in_channels,embed_dims,out_channels,mlp_ratios,drop_path,type):
        super().__init__()
        self.type = type
        # self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0)
        self.van = Van_block(in_chans = in_channels, embed_dims = embed_dims,mlp_ratios = mlp_ratios,drop_rate=drop_path)
        self.conv = nn.Conv2d(embed_dims, out_channels, kernel_size=1, padding=0)
...
class merge_c2f_t(nn.Module):
    def __init__(self, in_channels,embed_dims,out_channels,mlp_ratios,drop_path):
        super().__init__()
