...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio, drop,drop_path):
        super().__init__()
...
class c2f_Block(nn.Module):
    def __init__(self, dim, depths, drop_rate):
        super().__init__()
        mlp_ratio = 4
        dpr = [x.item() for x in torch.linspace(0, drop_rate, depths)]  # stochastic depth decay rule
        cur = 0
        self.block = nn.ModuleList([Block(
            dim=dim, mlp_ratio=mlp_ratio, drop=drop_rate, drop_path=dpr[cur + j])
